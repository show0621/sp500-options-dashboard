    ...
    for i in range(1, len(df)):
        ...
        if strategy_type == "MACD 動能共振":
            ...
            if not holding and (current['Close'] > pivot) and (current['Volume'] > 1.2 * current['Vol_SMA_20']) and (current['Close'] > current['SMA_60']):
                ...
                sell_signal, reason = True, '跌破20MA'
            ...
            if not holding and is_uptrend and (current['Low'] <= current['BB_Lower']):
                buy_signal, reason = True, '回檔觸及布林下軌'
            elif holding:
                if current['High'] >= current['BB_Upper']: sell_signal, reason = True, '觸及布林上軌'
...
def plot_chart(df, title, trade_log_df):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_Upper'], line=dict(color='rgba(52, 152, 219, 0.2)'), name='上軌'), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_Lower'], line=dict(color='rgba(52, 152, 219, 0.2)'), fill='tonexty', name='下軌'), row=1, col=1)
    if not trade_log_df.empty:
        buys = trade_log_df[trade_log_df['Action'] == 'BUY']
        sells = trade_log_df[trade_log_df['Action'] == 'SELL']
        fig.add_trace(go.Scatter(x=pd.to_datetime(buys['Date']), y=buys['Price']*0.95, mode='markers', marker=dict(symbol='triangle-up', size=12, color='red'), name='買進'), row=1, col=1)
        fig.add_trace(go.Scatter(x=pd.to_datetime(sells['Date']), y=sells['Price']*1.05, mode='markers', marker=dict(symbol='triangle-down', size=12, color='green'), name='賣出'), row=1, col=1)
    ...
    if os.path.exists('signals.csv'):
        try:
            ...
            if not signals_df.empty:
                # 根據下拉選單過濾
                filtered_df = signals_df[signals_df['符合策略'] == strategy_choice] if '符合策略' in signals_df.columns else signals_df
...
if ticker_input:
    ...
    if not df_daily_raw.empty:
        ...
        # --- 找回策略回測比較指標 ---
        st.markdown(f"### 📊 【{strategy_choice}】策略回測比較")
        if not trade_log_df.empty:
            sells = trade_log_df[trade_log_df['Action'] == 'SELL']
            c1, c2, c3 = st.columns(3)
            c1.metric("純買現股總利潤", f"{sells['現股 PnL(%)'].sum():.2f}%")
            c2.metric("Sell Put 總收租", f"{sells['Sell Put PnL(%)'].sum():.2f}%")
            c3.metric("Covered Call 總績效", f"{sells['Covered Call PnL(%)'].sum():.2f}%")
        
        st.plotly_chart(plot_chart(df_daily.tail(150), ticker_input, trade_log_df), use_container_width=True)
        ...
        with col_news:
            ...
            if news_data:
                for n in news_data:
                    # 使用更穩健的抓取方式
                    title = n.get('title') or "無標題新聞"
                    # 處理 Yahoo Finance 複雜的嵌套 Link
                    link = n.get('link')
                    if not link:
        ...
        with col_chips:
            ...
            if inst_holders is not None and not inst_holders.empty:
                try:
                    ...
                    st.info(f"機構持有比例推估：{pct:.1%}")
                    st.dataframe(inst_holders[['Holder', 'pctHeld']].head(5), hide_index=True)
                except: st.write("無法解析籌碼詳細數據。")
            else: st.write("暫無籌碼數據。")
