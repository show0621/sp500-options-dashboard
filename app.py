...
def plot_chart(df, title, trade_log_df):
    ...
    if not trade_log_df.empty:
        ...
        fig.add_trace(go.Scatter(x=pd.to_datetime(buys['Date']), y=buys['Price'] * 0.95, mode='markers+text', marker=dict(symbol='triangle-up', size=12, color='#E74C3C'), name='買進點', text="買", textposition="bottom center"), row=1, col=1)
    ...
    fig.update_layout(title=title, xaxis_rangeslider_visible=False, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(family="Noto Serif TC", size=12, color="#2C3E50"), margin=dict(l=20, r=20, t=50, b=20))
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(200,200,200,0.2)')
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(200,200,200,0.2)')
    return fig

# ==========================================
# 前端 UI 渲染 (恢復原始 Sidebar 與 Layout)
# ==========================================
    ...
    try:
        if os.path.exists('signals.csv'):
            ...
            if not signals_df.empty:
                ...
                else: st.write(f"今日無符合【{strategy_choice}】條件之標的。")
            else: st.write("今日市場無符合條件之標的。")
        else:
            st.warning("尚未偵測到 `signals.csv`。")
            st.caption("請確認 GitHub Actions 的掃描腳本是否已經成功執行並寫入資料庫。")
    except Exception as e: st.error(f"讀取推薦清單時發生錯誤: {e}")
...
if ticker_input:
    ...
    if not df_daily_raw.empty:
        ...
        if not trade_log_df.empty and len(trade_log_df[trade_log_df['Action'].isin(['SELL', 'HOLDING'])]) > 0:
            sells = trade_log_df[trade_log_df['Action'].isin(['SELL', 'HOLDING'])]
            stock_total, stock_win_rate = sells['現股 PnL(%)'].sum(), (len(sells[sells['現股 PnL(%)'] > 0]) / len(sells)) * 100
            put_total, put_win_rate = sells['Sell Put PnL(%)'].sum(), (len(sells[sells['Sell Put PnL(%)'] > 0]) / len(sells)) * 100
            cc_total, cc_win_rate = sells['Covered Call PnL(%)'].sum(), (len(sells[sells['Covered Call PnL(%)'] > 0]) / len(sells)) * 100
        else: stock_total, stock_win_rate, put_total, put_win_rate, cc_total, cc_win_rate = 0, 0, 0, 0, 0, 0

        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown(f'<div class="metric-card"><h4>📈 1. 純買現股 (Buy & Hold)</h4><h2>總利潤: {stock_total:.2f}%</h2><p>歷史勝率: <b>{stock_win_rate:.1f}%</b></p><p style="font-size: 0.85em; color: gray;">完全承擔漲跌幅，大漲時利潤最高，下跌時無防護。</p></div>', unsafe_allow_html=True)
        with col2:
            st.markdown(f'<div class="metric-card"><h4>🛡️ 2. 賣出賣權 (Sell Put)</h4><h2>總利潤: {put_total:.2f}%</h2><p>歷史勝率: <b>{put_win_rate:.1f}%</b></p><p style="font-size: 0.85em; color: gray;">空手收租。防禦力最強，只要不暴跌都能賺取固定權利金。</p></div>', unsafe_allow_html=True)
        with col3:
            st.markdown(f'<div class="metric-card"><h4>⚖️ 3. 掩護性買權 (Covered Call)</h4><h2>總利潤: {cc_total:.2f}%</h2><p>歷史勝率: <b>{cc_win_rate:.1f}%</b></p><p style="font-size: 0.85em; color: gray;">持有股票並賣出買權。降低持股成本，但犧牲大漲時的尾部利潤。</p></div>', unsafe_allow_html=True)
        ...
        else: st.write("過去一年無符合條件之交易訊號。")
            
        # ==========================================
        # 修正新聞報錯區域 (保持原始樣式，僅修正邏輯)
        # ==========================================
        st.markdown("---")
        ...
        with col_news:
            ...
            if news_data:
                for n in news_data:
                    # 核心修正：使用 (n.get('content') or {}) 確保即便 Key 存在但值為 None 也不會崩潰
                    c = n.get('content') or {}
        ...
        with col_chips:
            ...
            else: st.write("暫無機構籌碼資料。")
    else: st.error("無法尋獲該代碼的軌跡，請確認美股代碼是否正確。")
